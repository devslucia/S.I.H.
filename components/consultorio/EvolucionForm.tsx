...
import { Plus, CheckCircle, Clock, User, Send } from "lucide-react";
...
export function EvolucionForm({ apiBase }: EvolucionFormProps) {
  ...
  const fetchEvoluciones = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${apiBase}/evoluciones`);
  ...
  useEffect(() => { fetchEvoluciones(); }, [apiBase]);
          ...
          {evoluciones.map((evo) => (
            <div key={evo.id} className="card p-4">
              ...
              <div className="flex items-center gap-1 mt-2 text-xs text-muted">
                <User size={12} /> {evo.usuario.apellido ? `${evo.usuario.apellido}, ${evo.usuario.nombre}` : evo.usuario.nombre} ({evo.usuario.rol})
              </div>
            </div>
          ))}
        </div>
