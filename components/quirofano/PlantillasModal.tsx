...
import { BookMarked, Check, Download, FileUp, Loader, Pencil, Plus, Trash2 } from "lucide-react";
...
interface PlantillasModalProps {
  open: boolean;
  onClose: () => void;
  onUsar: (nombre: string, descripcion: string) => void;
  onListChanged: () => void;
}
...
export function PlantillasModal({ open, onClose, onUsar, onListChanged }: PlantillasModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Mis Plantillas de Protocolo Quirúrgico" size="full">
      {view === "list" ? (
        <div className="space-y-4">
          ...
          {loading ? (
            ...
            <p className="text-xs text-muted">Todavía no tenés plantillas guardadas.</p>
          ) : (
                ...
                <div key={p.id} className="rounded-lg border border-border p-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => { onUsar(p.nombre, p.descripcion ?? ""); onClose(); }} className={btnTeal} title="Cargar en el protocolo">
                        <Download size={14} /> Usar
                      </button>
