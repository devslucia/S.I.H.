...
import { HeartPulse, AlertTriangle, Activity, ChevronDown, ChevronUp,
  ChevronRight,
  FileText, Pill, Plus,
} from "lucide-react";
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-[13px] font-medium text-text flex items-center gap-2">
          <Pill size={15} className="text-brand" /> Medicación adicional
        </h4>
        <button
          onClick={() => setShowModal(true)}
          className="py-1.5 px-3 inline-flex items-center gap-1.5 bg-brand-soft text-brand border border-brand hover:bg-brand-soft/80 font-medium rounded-lg transition-colors text-[12px]"
        >
          <Plus size={14} /> Agregar medicación adicional
        </button>
      </div>

      {loading ? (
        <div className="p-4 text-center text-muted text-[13px]">Cargando...</div>
      ) : aplicaciones.length === 0 ? (
        <div className="p-4 text-center border border-dashed border-border rounded-lg">
          <Pill size={32} className="mx-auto text-muted/30 mb-2" />
          <p className="text-muted text-[13px]">Sin medicación adicional registrada</p>
          <p className="text-xs text-muted mt-1">Usá &ldquo;Agregar medicación adicional&rdquo; para cargar la primera</p>
        </div>
      ) : (
        <div className="divide-y divide-border">
          {aplicaciones.map((app) => (
            <div key={app.id} className="p-3 hover:bg-surface-hover transition-colors">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-medium text-text truncate text-[13px]">
                      {app.stockItem?.nombre || "Medicación sin stock"}
                      {app.stockItem?.nTroquel && (
                        <span className="ml-2 text-[10px] font-mono text-muted px-1.5 py-0.5 rounded bg-background">{app.stockItem.nTroquel}</span>
                      )}
                    </p>
                    <p className="text-xs text-muted mt-0.5 flex flex-wrap gap-3">
                      ...
                      {app.motivo && <span className="truncate max-w-[250px]">Motivo: {app.motivo}</span>}
                    </p>
                  </div>
                </div>
...
export default function EnfermeriaPage() {
  ...
  const [adHocMap, setAdHocMap] = useState<Record<string, AdHocAplicacion[]>>({});
  const [loadingAdHoc, setLoadingAdHoc] = useState<Record<string, boolean>>({});
  ...
  const fetchAdHoc = useCallback(async (internacionId: string) => {
    setLoadingAdHoc((prev) => ({ ...prev, [internacionId]: true }));
    try {
      const res = await fetch(`/api/historia-clinica/${internacionId}/enfermeria/ad-hoc`);
      if (res.ok) {
        const data = await res.json();
        setAdHocMap((prev) => ({ ...prev, [internacionId]: Array.isArray(data) ? data : [] }));
      }
    } catch {} finally {
      setLoadingAdHoc((prev) => ({ ...prev, [internacionId]: false }));
    }
  }, []);
  ...
  }, [selectedInternacion, fetchControles, fetchAdHoc]);
  ...
  }, [focusId, internaciones, fetchControles, fetchAdHoc]);
              ...
              <div key={i.id} id={`internacion-${i.id}`} className="border border-border rounded-lg overflow-hidden bg-surface">
                ...
                {selectedInternacion === i.id && (
                  <div className="p-4 border-t border-border space-y-4">
                    ...
                    <MedicacionAdicional
                      internacionId={i.id}
                      aplicaciones={adHocMap[i.id] || []}
                      loading={loadingAdHoc[i.id]}
                      onAdd={() => fetchAdHoc(i.id)}
                    />
